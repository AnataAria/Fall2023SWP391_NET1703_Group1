<script lang="ts">
  ...
  function disableSubmitButton() {
    (document.getElementById("submitButton") as HTMLButtonElement).hidden =
      true;
  ...
  >
    <form class="space-y-6" autocomplete="off">
      ...
      <div>
        <label
          for="password"
          class="peer block mb-2 text-sm font-medium text-gray-900 dark:text-white after:content-['*'] after:ml-0.5 after:text-red-500"
          >Password</label
        >
        <input
          type="password"
          ...
          name="password"
          id="password"
          class="peer-invalid bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
        />
      </div>
      <div>
        <label
      ...
      <div id="submitButton">
        <Button
          ...
          type="submit"
          class="w-full"
          on:click={validatePassword}>Reset Password</Button
        >
      </div>
