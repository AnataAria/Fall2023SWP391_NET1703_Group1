<script lang="ts">
    ...
    res = axios
      ...
      .then((response: AxiosResponse) => {
        if (response.status === 200) {
          ...
          totalRevenue = CurrencyHandler(
            monthlySalary.firstMonth +
  ...
  async function getCsvSalary() {
    try {
      var year = new Date().getFullYear();
      var month = new Date().getMonth() + 1;
      const response = await axios.get(
        apiBaseUrl + `salarys/exports?year=${year}&month=${month}`,
        {
          headers: {
  ...
  >
    <div class="flex justify-between items-center pt-5">
      <Button
        ...
        >Yearly Profits<ChevronDownSolid class="w-2.5 m-2.5 ml-1.5" /></Button
      >
      <Dropdown class="w-40" offset="-6">
        ...
        <DropdownItem
          on:click={() => {
            GetMonthlyData(LastYear);
          }}>Last Year</DropdownItem
        >
        <DropdownItem
          on:click={() => {
            GetMonthlyData(TwoYearsAgo);
          }}>2 Years Ago</DropdownItem
        >
      </Dropdown>
      <Button
        on:click={() => {
